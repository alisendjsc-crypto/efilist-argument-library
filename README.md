...
The binding integrity source is the project canon — `project_canon_v38_22.json`, whose `session_log_recent` and `keyset_delta_ledger` record each pin move with its md5 and byte count (the `archive_attestation` block stops at the v3.7 line) — together with the wuld.ink pin tooling.
