#!/usr/bin/env python3
"""transgenderism_validator_v0_1.py — Transgenderism corpus/ledger/export validator.
...
CHECKS:
  ...
  5. export determinism   — build the index twice via build_transgenderism_index;
...
Usage:
  python3 transgenderism_validator_v0_1.py --self-test     # synthetic + live, JSON, 0/1
  python3 transgenderism_validator_v0_1.py <corpus.json>   # validate one corpus file
"""
...
def check_schema(corpus):
    ...
    for o in objs:
        ...
        if s not in STRAND_VALUES:
            out.append(_v("schema", "strand %r not in {harm,consent,sovereignty}" % s, oid))
        r = o.get("rebuttal")
...
def check_export_determinism(corpus):
    ...
    try:
        mod = importlib.import_module("build_transgenderism_index")
    except Exception as e:  # pragma: no cover
        return [_v("export", "cannot import build_transgenderism_index: %r" % e)]
    try:
...
def check_keyword_projection(corpus):
    ...
    try:
        ...
        return [_v("keywords", "cannot import build_transgenderism_index: %r" % e)]
    try:
        payload = mod.build(corpus)
    except SystemExit as e:
...
def run_synthetic_tests():
    ...
    # strand enum (3-strand verdict as a guard)
    c = _good_corpus(); c["objections"][0]["strand"] = "harm"
    cases["strand_harm_passes"] = (len(check_schema(c)) == 0, True)
    c = _good_corpus(); c["objections"][0]["strand"] = "consent"
    cases["strand_consent_passes"] = (len(check_schema(c)) == 0, True)
...
def _main(argv):
    ...
    ledger = {}
    lp = os.path.join(os.path.dirname(os.path.abspath(argv[1])), "transgenderism_grading_ledger.json")
    if os.path.exists(lp):
        ledger = json.load(open(lp, encoding="utf-8"))
    issues = validate_all(corpus, ledger)
