#!/usr/bin/env python3
"""build_abortion_index.py — Abortion objections-index export generator.
...
breaking shape change so the consumer can detect it):
  shape        : {"schema_version": <int>, "library": "abortion",
  ...
  surface_route: deep-link base; the consumer builds <surface_route>#obj-<id>
                 (i.e. "abortion/combined#obj-<id>"). Self-describing so the
...
Usage:
  python3 build_abortion_index.py [WING_DIR=../site/abortion] [--out abortion-objections-index.json]
  python3 build_abortion_index.py --check   # validate only, write nothing
"""
import sys, os, json, glob, argparse

SCHEMA_VERSION = 1
LIBRARY = "abortion"
SURFACE_ROUTE = "abortion/combined"
GLOSS_MAX = 200
OUT_NAME = "abortion-objections-index.json"
HERE = os.path.dirname(os.path.abspath(__file__))
# the served wing (WI-K360, 2026-09-20): the corpus the page fetches lives there; the index stays beside this script
SITE = os.path.normpath(os.path.join(HERE, "..", "site", "abortion"))
...
def find_corpus(root):
    pats = ["abortion_corpus_v*.json", "abortion_corpus*.json"]
    for pat in pats:
