#!/usr/bin/env python3
"""abortion_validator_v0_1.py — Abortion corpus/ledger/export validator.
...
CHECKS:
  1. schema validity      — corpus shape + required node fields + count/response
                            self-consistency + strand-enum {harm,consent,sovereignty}.
  ...
  5. export determinism   — build the index twice via build_abortion_index;
...
Usage:
  python3 abortion_validator_v0_1.py --self-test     # synthetic + live, JSON, 0/1
  python3 abortion_validator_v0_1.py <corpus.json>   # validate one corpus file
"""
import sys, os, json, re, importlib

HERE = os.path.dirname(os.path.abspath(__file__))
SITE = os.path.normpath(os.path.join(HERE, "..", "site", "abortion"))  # the served wing (WI-K360, 2026-09-20)
...
def check_schema(corpus):
    ...
    for o in objs:
        ...
        if not str(o.get("objection", "")).strip():
        ...
        s = o.get("strand")
        if s not in STRAND_VALUES:
            out.append(_v("schema", "strand %r not in %s" % (s, sorted(STRAND_VALUES)), oid))
        ly = o.get("layer")
        if ly not in (1, 2):
...
def check_band_geomean(corpus, ledger):
    ...
    for gid, g in grades.items():
        ...
        lg = long_block.get("grade")
        if gm is not None and lg in ("A", "B", "C", "D") and band(gm, thr) != lg and not _band_ambiguous(gm, thr):
            out.append(_v("band", "long grade %s disagrees with band(axes-geomean %.4f)=%s (derive-band-at-regen)" % (lg, gm, band(gm, thr)), gid))
    return out
...
def check_export_determinism(corpus):
    ...
    try:
        mod = importlib.import_module("build_abortion_index")
    except Exception as e:  # pragma: no cover
        return [_v("export", "cannot import build_abortion_index: %r" % e)]
    try:
...
def check_keyword_projection(corpus):
    ...
    try:
        ...
        return [_v("keywords", "cannot import build_abortion_index: %r" % e)]
    try:
        payload = mod.build(corpus)
    except SystemExit as e:
...
def _good_corpus():
    return {
        ...
        "objections": [{
            "id": "alpha", "tier": 1, "strand": "bodily-sovereignty", "layer": 1, "role": "spine",
...
def run_synthetic_tests():
    ...
    c = _good_corpus(); c["objections"][0]["rebuttal"] = {"short": ""}
    cases["schema_missing_depth_fires"] = (len(check_schema(c)) > 0, True)
    cases["schema_clean_passes"] = (len(check_schema(_good_corpus())) == 0, True)
    c = _good_corpus(); del c["objections"][0]["move_tags"]
    cases["schema_missing_move_tags_fires"] = (len(check_schema(c)) > 0, True)
    c = _good_corpus(); del c["objections"][0]["objection"]
    cases["schema_missing_objection_fires"] = (len(check_schema(c)) > 0, True)

    # strand enum (seeded {bodily-sovereignty}; grows as nodes land) + layer enum (L137)
    c = _good_corpus(); c["objections"][0]["strand"] = "bodily-sovereignty"
    cases["strand_bodilysov_passes"] = (len(check_schema(c)) == 0, True)
    c = _good_corpus(); c["objections"][0]["strand"] = "fetal-status"
    ...
    cases["layer_missing_fires"] = (len(check_schema(c)) > 0, True)
    ...
    # band (incl. derive-band-at-regen long-axes check)
    ...
    led = {"grades": {"alpha": {"axes": {"v": .95, "s": .95, "c": .95, "r": .95, "a": .95},
                                "short": {"rsi_pct": 95.0, "grade": "A"}, "medium": {"rsi_pct": 95.0, "grade": "A"},
                                "long": {"rsi_pct": 88.0, "grade": "B"}, "headline_grade_long": "B"}}}
    cases["band_axes_long_mismatch_fires"] = (len(check_band_geomean(_good_corpus(), led)) > 0, True)

    # rwe referential integrity + enforcement (trans rwe[] ref slot)
    def _rwe(**kw):
    ...
    # keyword projection: trans source = keywords if present else move_tags
    ...
    cases["kw_no_source_passes"] = (len(_keyword_projection_violations(_good_corpus(), {"objections": [{"id": "alpha"}]})) == 0, True)
    _mt = _good_corpus(); _mt["objections"][0]["move_tags"] = ["m1", "m2"]
    cases["kw_movetags_projected_passes"] = (len(_keyword_projection_violations(_mt, {"objections": [{"id": "alpha", "keywords": ["m1", "m2"]}]})) == 0, True)
    cases["kw_movetags_dropped_fires"] = (len(_keyword_projection_violations(_mt, {"objections": [{"id": "alpha"}]})) > 0, True)
...
def run_live_test():
    ...
    try:
        corpus = json.load(open(os.path.join(SITE, "abortion_corpus_v0_1.json"), encoding="utf-8"))
    except Exception as e:
        return {"_overall_pass": False, "error": "cannot load seed corpus: %r" % e}
    ledger = {}
    lp = os.path.join(SITE, "abortion_grading_ledger.json")
...
def _main(argv):
    ...
    ledger = {}
    lp = os.path.join(os.path.dirname(os.path.abspath(argv[1])), "abortion_grading_ledger.json")
    if os.path.exists(lp):
        ledger = json.load(open(lp, encoding="utf-8"))
    issues = validate_all(corpus, ledger)
