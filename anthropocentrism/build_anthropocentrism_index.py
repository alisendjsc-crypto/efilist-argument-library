#!/usr/bin/env python3
"""build_anthropocentrism_index.py — Anthropocentrism objections-index export generator.
...
breaking shape change so the consumer can detect it):
  shape        : {"schema_version": <int>, "library": "anthropocentrism",
  ...
  surface_route: deep-link base; the consumer builds <surface_route>#obj-<id>
                 (i.e. "anthropocentrism/combined#obj-<id>"). Self-describing so the
...
Usage:
  python3 build_anthropocentrism_index.py [WING_DIR=../site/anthropocentrism] [--out anthropocentrism-objections-index.json]
  python3 build_anthropocentrism_index.py --check   # validate only, write nothing
"""
import sys, os, json, glob, argparse

SCHEMA_VERSION = 1
LIBRARY = "anthropocentrism"
SURFACE_ROUTE = "anthropocentrism/combined"
GLOSS_MAX = 200
OUT_NAME = "anthropocentrism-objections-index.json"
HERE = os.path.dirname(os.path.abspath(__file__))
# the served wing (WI-K360, 2026-09-20): the corpus the page fetches lives there; the index stays beside this script
SITE = os.path.normpath(os.path.join(HERE, "..", "site", "anthropocentrism"))


def fail(msg):
    sys.stderr.write("ANTHRO-INDEX FAIL: " + msg + "\n")
    sys.exit(1)


def find_corpus(root):
    pats = ["anthropocentrism_corpus_v*.json", "anthropocentrism_corpus*.json"]
    for pat in pats:
