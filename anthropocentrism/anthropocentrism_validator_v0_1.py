#!/usr/bin/env python3
"""anthropocentrism_validator_v0_1.py — Anthropocentrism corpus/ledger/export validator.
...
CHECKS:
  ...
  5. export determinism   — build the index twice via build_anthropocentrism_index;
  ...
  8. axis vocabulary      -- for a registered wing (wing 2 = Anthropocentrism) every
                            node carries >=1 move_tag from that wing's fittingness axes
                            (the embedded fittingness-axis tag; K147 schema ruling).


Genesis (Cowork K147, 2026-06-26): forked from the right-to-die validator. Wing-2 deltas:
top-level `strands` -> `fittingness_axes`; per-node `access_basis` dropped (Wing-1-local);
embedded fittingness-axis tag carried inside `move_tags[]`, controlled vocab keyed off corpus
`wing` (check_axis_vocab). RWE schema vendored at anthropocentrism_rwe_schema_v0_1.json.

Usage:
  python3 anthropocentrism_validator_v0_1.py --self-test     # synthetic + live, JSON, 0/1
  python3 anthropocentrism_validator_v0_1.py <corpus.json>   # validate one corpus file
"""
import sys, os, json, re, importlib

HERE = os.path.dirname(os.path.abspath(__file__))
SITE = os.path.normpath(os.path.join(HERE, "..", "site", "anthropocentrism"))  # the served wing (WI-K360, 2026-09-20)
...
def check_export_determinism(corpus):
    ...
    try:
        mod = importlib.import_module("build_anthropocentrism_index")
    except Exception as e:  # pragma: no cover
        return [_v("export", "cannot import build_anthropocentrism_index: %r" % e)]
    try:
...
def check_keyword_projection(corpus):
    ...
    try:
        ...
        return [_v("keywords", "cannot import build_anthropocentrism_index: %r" % e)]
    try:
        payload = mod.build(corpus)
    except SystemExit as e:
...
def check_axis_vocab(corpus):
    """Wing-keyed controlled vocabulary for the embedded fittingness-axis tag.
    Per the K147 schema ruling a node's fittingness axis is embedded inside
    move_tags[] (no parallel field); the corpus-level `wing` selects the vocab.
    For a registered wing (wing 2 = Anthropocentrism) every node must carry >=1
    move_tag drawn from that wing's fittingness axes -- its declared axis. Wings
    with no registered vocab are not checked."""
    out = []
    if not isinstance(corpus, dict):
        return out
    vocab = WING_AXIS_VOCAB.get(corpus.get("wing"))
    if not vocab:
        return out
    for o in corpus.get("objections", []):
        if not isinstance(o, dict):
        ...
        if not isinstance(mt, list):
            continue  # schema check already flags a non-list move_tags
        if not (set(mt) & vocab):
            out.append(_v("axis", "wing-%s node carries no fittingness-axis move_tag from %s"
                          % (corpus.get("wing"), sorted(vocab)), oid))
    return out
...
def validate_all(corpus, ledger):
    ...
    issues += check_axis_vocab(corpus)
...
def _good_corpus():
    return {
        ...
        "objections": [{
            ...
            "trigger": "t", "keywords": [], "move_tags": ["anti-mandate"], "diagnosis": "d",
...
def run_synthetic_tests():
    ...
    # axis vocabulary (wing-keyed embedded fittingness axis)
    cases["axis_vocab_clean_passes"] = (len(check_axis_vocab(_good_corpus())) == 0, True)
    c = _good_corpus(); c["objections"][0]["move_tags"] = ["some-local-label"]
    cases["axis_vocab_missing_fires"] = (len(check_axis_vocab(c)) > 0, True)
    c = _good_corpus(); c["wing"] = 1; c["objections"][0]["move_tags"] = []
    cases["axis_vocab_unregistered_wing_skips"] = (len(check_axis_vocab(c)) == 0, True)
...
def run_live_test():
    ...
    try:
        corpus = json.load(open(os.path.join(SITE, "anthropocentrism_corpus_v0_1.json"), encoding="utf-8"))
    except Exception as e:
        return {"_overall_pass": False, "error": "cannot load seed corpus: %r" % e}
    ledger = {}
    lp = os.path.join(SITE, "anthropocentrism_grading_ledger.json")
...
def _main(argv):
    ...
    ledger = {}
    lp = os.path.join(os.path.dirname(os.path.abspath(argv[1])), "anthropocentrism_grading_ledger.json")
    if os.path.exists(lp):
        ledger = json.load(open(lp, encoding="utf-8"))
    issues = validate_all(corpus, ledger)
