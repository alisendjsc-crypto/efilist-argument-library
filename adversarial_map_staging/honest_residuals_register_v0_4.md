# Honest-Residuals Register v0.4

**Status: WORKING, and derived from a terminal artifact.** 40 honest-residue (d) entries consolidated from `adversarial_map_v1_3.json` (68182741), which is the successor assembly.
Contributing phases: A (6), B1 (5), B2 (12), C (3), D (4), E (2), F (7), G (1). Contributing nothing: R.
Source corpus `04bf6482` / 82 nodes. Rebuild: `python3 build_register_v0_4.py`.
...
Every bedrock except HR-15 and HR-06 is byte-identical to v0_3 field for field, asserted before the write rather than claimed after it: 13 of 15 checked and unchanged. HR-15 is new; HR-06 gains exactly one relation and changes in no other way. The v0_1 gate above is restricted to the phases v0_1 read and is structurally blind to a change confined to R/F/G, which is why this second gate exists.
...
9 candidates derived from the two signals, 16 relations declared, 7 declared beyond what the signals detect.
...
- **unchosen-relations-bind-without-authorization** — `care-ethics` (G)
